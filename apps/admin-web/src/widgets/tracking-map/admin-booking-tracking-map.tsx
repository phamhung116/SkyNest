...
type AdminBookingTrackingMapProps = {
  booking: Booking;
...
const pilotCarIcon = divIcon({
  className: "admin-map-marker admin-map-marker--car",
...
const destinationIcon = divIcon({
  className: "admin-map-marker admin-map-marker--destination",
...
const dedupePoints = (points: Array<MapPoint | null | undefined>) => {
  ...
  for (const point of points) {
    if (point && !unique.some((item) => isSamePoint(item, point))) {
      unique.push(point);
    }
  }
  return unique;
...
const parseRoutePoints = (tracking: Tracking | null) =>
  (tracking?.route_points ?? [])
    .map((point) => parseMapPoint(point, "Điểm lộ trình"))
...
export const AdminBookingTrackingMap = ({ booking, tracking }: AdminBookingTrackingMapProps) => {
  const routePoints = useMemo(() => parseRoutePoints(tracking), [tracking]);
  const currentLocation =
    parseMapPoint(tracking?.current_location, "Vị trí hiện tại") ?? routePoints[routePoints.length - 1] ?? null;
  ...
  const center = currentLocation ?? markers[markers.length - 1] ?? { lat: 16.0544, lng: 108.2022, name: "Đà Nẵng" };
  ...
  return (
    <div className="admin-booking-map">
      <MapContainer center={[center.lat, center.lng]} zoom={13} scrollWheelZoom={false} style={{ height: "100%", width: "100%" }}>
        ...
        {routePositions.length > 1 ? <Polyline positions={routePositions} color="#2563eb" weight={5} opacity={0.9} /> : null}
      ...
      {!viewportPoints.length ? (
        <div className="admin-booking-map__empty">Lộ trình sẽ hiển thị tại đây khi phi công bắt đầu theo dõi GPS.</div>
      ) : null}
    </div>
  );
};
